...
class Policy_Gradient_pair_model(object):
    ...
    def load_model(self, args, prefix=""):
        info = str(args.dataset) + '_' + str(args.threshold) + '_' + str(args.mu) + '_' + str(args.nu) + '_' + str(args.trail)
        self.policy.load_state_dict(torch.load(os.path.join("./output", f"{prefix}policy_{info}.pth"), map_location='cuda:0'))
        self.classifier.load_state_dict(torch.load(os.path.join("./output", f"{prefix}classifier_{info}.pth"), map_location='cuda:0'))
